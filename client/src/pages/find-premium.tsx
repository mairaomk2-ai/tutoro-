...
export default function FindPremium() {
  ...
  const { data: nearbyProfiles = [], isLoading: nearbyLoading } = useQuery<ProfileCard[]>({
    ...
    enabled: activeTab === "nearest" && !!user && user.userType === 'student' || user?.userType === 'teacher',
  });
  ...
  const { data: featuredProfiles = [] } = useQuery<ProfileCard[]>({
    queryKey: [`/api/search/featured/${isStudent ? 'teachers' : 'students'}`],
    queryFn: async () => {
      // For now, just return the same results as regular search
      const response = await apiRequest('GET', `/api/search/${isStudent ? 'teachers' : 'students'}`);
